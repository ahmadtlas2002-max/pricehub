...
(async function handlePaymentSuccess(){
    ...
    const order = {
        orderId: "PH-" + Math.floor(10000 + Math.random() * 90000),
        customerName: "دفع إلكتروني",
        customerPhone: "غير موجود",
        customerEmail: (document.getElementById("userBox")?.innerText || "")
            .replace("👤","")
            .trim(),
        items: paidCart,
        subtotal: paidCart.reduce((sum,item)=>sum + Number(item.price || 0),0),
        discountPercent: 0,
        discountAmount: 0,
        coupon: "",
        total: paidCart.reduce((sum,item)=>sum + Number(item.price || 0),0),
        date: new Date().toLocaleString(),
        status: "جديد",
